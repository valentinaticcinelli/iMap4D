function descriptive_part4D(varargin)
...
mkdir([pathname, '\Descriptive_STAT']);
%%
if exist('DescriptvM','var') && exist('FixMap','var')
    ...
    h1 = figure('NumberTitle','off','Name','Eye movement measurement distribution',...
        'position',[xpos,ypos,screensize(3)/2,screensize(4)*3/4]);
    ...
    if ismac
        saveas(h1,strcat(pathname,'Descriptive_STAT\Eye_Mvt_Measure_Dist'),'png')
    else
        saveas(h1,strcat(pathname,'Descriptive_STAT/Eye_Mvt_Measure_Dist'),'png')
    end
    ...
    h2 = figure('NumberTitle','off','Name','Mean Fixation Intensity',...
        'position',[xpos,ypos,screensize(3),screensize(4)*3/4]);
    title('Mean Fixation')

    TempMap=squeeze(nanmean(FixMap,1));                
    heatmapObj4D(obj,TempMap)
                
    axis off,axis equal
    if ismac
        saveas(h2,strcat(pathname,'Descriptive_STAT\Mean_Intensity_Map'),'png')
    else
        saveas(h2,strcat(pathname,'Descriptive_STAT/Mean_Intensity_Map'),'png')
    end
    %close(h1,h2)
    ...
    for ic=1:length(CName)
        ...
        title('Path Length(MEAN)')
        mkdir([pathname, 'Descriptive_STAT/', CName{ic}])
        if ismac
            saveas(h1,strcat(pathname,'Descriptive_STAT\',CName{ic},'\',CName{ic},'_Boxplot'),'png')
        else
            saveas(h1,strcat(pathname,'Descriptive_STAT/',CName{ic},'/',CName{ic},'_Boxplot'),'png')
        end
        %close(h1)
    ...
    for ic=1:length(CName)-1
        ...
        if lengthcondi>maxsub
            ...
            for isub=1:lengthcondi
                ...
                if pp==1
                    if ismac
                        saveas(h(k-1),strcat('Descriptive_STAT\',CName{ic},'\',CName{ic},'_Mean_Intensity_Map',num2str(k-1)),'png')
                    else
                        saveas(h(k-1),strcat('Descriptive_STAT/',CName{ic},'/',CName{ic},'_Mean_Intensity_Map',num2str(k-1)),'png')
                    end
            ...
            if ismac
                saveas(h(k),strcat(pathname,'Descriptive_STAT\',CName{ic},'\',CName{ic},'_Mean_Intensity_Map',num2str(k)),'png')
            else
                saveas(h(k),strcat(pathname,'Descriptive_STAT/',CName{ic},'/',CName{ic},'_Mean_Intensity_Map',num2str(k)),'png')
            end
            %close(gcf)
            ...
            % save figures
            if ismac
                saveas(h(1),strcat(pathname,'Descriptive_STAT\',CName{ic},'\',CName{ic},'_Mean_Intensity_Map'),'png')
            else
                saveas(h(1),strcat(pathname,'Descriptive_STAT/',CName{ic},'/',CName{ic},'_Mean_Intensity_Map'),'png')
            end
            %%close figure
    ...
    while isempty(CName2)==0
        ...
        if isempty(CName2)==0
            ...
            if ismac
                mkdir([pathname,'Descriptive_STAT\',CNameall]);
                addpath([pathname,'Descriptive_STAT\',CNameall]);
            else
                mkdir([pathname,'Descriptive_STAT/',CNameall]);
                addpath([pathname,'Descriptive_STAT/',CNameall]);
            end
            ...
            if lengthcondi>maxsub
                ...
                for isub=1:lengthcondi
                    ...
                    if pp==1
                        if ismac
                            saveas(h(k-1),strcat(pathname,'Descriptive_STAT\',CNameall,'\',CNameall,'_Mean_Intensity_Map',num2str(k-1)),'png')
                        else
                            saveas(h(k-1),strcat(pathname,'Descriptive_STAT/',CNameall,'/',CNameall,'_Mean_Intensity_Map',num2str(k-1)),'png')
                        end
                ...
                if ismac
                    saveas(h(k),strcat(pathname,'Descriptive_STAT\',CNameall,'\',CNameall,'_Mean_Intensity_Map',num2str(k)),'png')
                else
                    saveas(h(k),strcat(pathname,'Descriptive_STAT/',CNameall,'/',CNameall,'_Mean_Intensity_Map',num2str(k)),'png')
                end
                %close(gcf)
                ...
                if ismac
                    saveas(h(1),strcat(pathname,'Descriptive_STAT\',CNameall,'\',CNameall,'_Mean_Intensity_Map'),'png')
                else
                    saveas(h(1),strcat(pathname,'Descriptive_STAT/',CNameall,'/',CNameall,'_Mean_Intensity_Map'),'png')
                end
                %close(gcf);
            ...
            title('Path Length(MEAN)')
            if ismac
                saveas(h(1),strcat(pathname,'Descriptive_STAT\',CNameall,'\',CNameall,'_Boxplot'),'png');
            else
                saveas(h(1),strcat(pathname,'Descriptive_STAT/',CNameall,'/',CNameall,'_Boxplot'),'png');
            end
            %close(gcf)
