function model_plot4D(obj,LMMmap,FixMap,pathname)
...
uiwait(gcf)
    ...
    function model(~,~)
        ...
        %perform contrast
        StatMap=imapLMMcontrast4D(LMMmap,opt1);
        ...
        imapLMMdisplay4D(obj,StatMap,normalized,backgroundfile(2:end-1),cmap(2:end-1),colormaprange,distplot,pathname);
    end
    ...
    function anova(~,~)
        ...
        % perform contrast
        StatMap=imapLMMcontrast4D(LMMmap,opt);
...
%%%%%%%        StatMap_c=multiple_comparison_tab(LMMmap,FixMap,StatMap,pathname);
    ...
    function display_results(~,~)
        ...
        if isempty(fullpathname)
            ...
            %output figure;
            % imapLMMdisplay(StatMap,0)
            imapLMMdisplay4D(obj,StatMaptoplot,normalized,backgroundfile(2:end-1),cmap(2:end-1),colormaprange,distplot,pathname);
        end
