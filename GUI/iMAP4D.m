function varargout = iMAP4D(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @iMAP4D_OpeningFcn, ...
    'gui_OutputFcn',  @iMAP4D_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function iMAP4D_OpeningFcn(hObject, event, handles, varargin)
...
function varargout = iMAP4D_OutputFcn(hObject, ~, handles)
...
if strcmp(answer,'Linear Contrast')==1;
    ...
    if iscell(filename)==0 %user selects only multiple file
        ...
        if isempty(fullpathname)
            ...
            if exist('LMMmap','var')&& exist('FixMap','var')
                model_plot4D(obj,LMMmap,FixMap,pathname);
            else
...
else  %visualize a statmap alreay created
