...
tmpPXint = FixMap(:,indextmp);
tbl.PixelIntensity = tmpPXint(:,1);
if isa(tbl,'dataset')
...
AnovaFStat              = NaN(size(statexample.Term,1),              ySize2);
Covb                    = NaN([size(lmexample.CoefficientCovariance), ySize2]);
MSEmatrix               = NaN(ySize2);
SSEmatrix               = MSEmatrix;
SSTmatrix               = MSEmatrix;
SSRmatrix               = MSEmatrix;
RsquaredMatrix          = NaN(2,ySize2);
ModelCriterionMatrix    = NaN(4,ySize2);% AIC,BIC,LogLikelihood,Deviance
CoefficientsMatrix      = NaN(length(lmexample.CoefficientNames), 2, ySize2);% save beta, SE
...
if singlep ~= 0
    ...
    if Ncatepred == 0
        ...
        CoefficientsMatrix2  = NaN(length(CatePredictor), 2,                     ySize2);% save beta, SE
        Covb2                = NaN(length(CatePredictor), length(CatePredictor), ySize2);
...
RandomStat                 = NaN(size(randstat,1), 2, ySize2);
